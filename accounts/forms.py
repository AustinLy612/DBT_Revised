...
class RegisterForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        if len(username) < 3:
            raise ValidationError(_("用户名至少需要 3 个字符"))
        return username
    ...
    def clean_invite_code(self):
        ...
        try:
            ...
            raise ValidationError(_("邀请码无效"))

        if invite.status == InviteCode.Status.USED:
            raise ValidationError(_("该邀请码已被使用"))
        if invite.status == InviteCode.Status.DISABLED:
            raise ValidationError(_("该邀请码已停用"))
    ...
    def clean(self):
        ...
        if pwd and pwd2 and pwd != pwd2:
            self.add_error("password_confirm", _("两次输入的密码不一致"))
        return data
