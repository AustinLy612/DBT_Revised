...
系统采用 MongoDB 作为主数据库，Qdrant 作为向量检索引擎，MiniMax 作为 AI 供应商（提供 LLM、TTS、图像生成能力），Redis 作为缓存与 Celery 消息队列，MinIO 作为对象存储。
...
| AI 模型 | MiniMax `MiniMax-M2.7` (LLM)、`speech-2.8-turbo` (TTS)、`image-01` (图像) |
...
├── media_app/        # MiniMax 图像生成、TTS 语音合成、ASR 语音识别服务
...
2. **AI 语义通道**（MiniMax LLM）：对中危关键词匹配 + 中度担忧指标（如"活得没意义"、"我恨我自己"）进行语义深度评估。
...
当 MiniMax LLM 不可用时（`APIError` 或任何异常），系统降级为**纯关键词模式**：
...
| `knowledge_base/rag/llm_client.py` | MiniMax LLM 客户端、APIError、ConfigurationError |
...
| 功能 | MiniMax 模型 | 服务函数 | 说明 |
|------|-------------|---------|------|
| 图像生成 | `image-01` | `generate_image()` | 用于教学情景配图、测试题配图。返回 URL（24 小时有效），不持久化图片文件。 |
| TTS 语音合成 | `speech-2.8-turbo` | `synthesize_speech()` | 将 AI 教学文本转为语音播报。支持返回音频 URL 或下载音频字节。 |
| ASR 语音识别 | MiniMax ASR | `transcribe_audio()` | 学生语音输入转文本。火山引擎作为 fallback（当前未实现）。 |
...
学生输入消息/答案
  ...
  → run_risk_assessment(text, keywords, context) — MiniMax LLM
  → 两通道独立判定
