...
系统采用 MongoDB 作为主数据库，Qdrant 作为向量检索引擎，MiniMax 作为 LLM 和图像生成供应商，火山引擎（Volcengine）作为 TTS 和 ASR 供应商，Redis 作为缓存与 Celery 消息队列，MinIO 作为对象存储。
...
| AI 模型 | MiniMax `MiniMax-M2.7` (LLM)、`image-01-live` (图像)、火山引擎 `BV700_streaming` (TTS 豆包语音合成2.0) |
